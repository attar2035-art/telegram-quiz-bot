...
def load_data():
    ...
    if not data.get("chapters"):
        raise ValueError("No chapters found in questions JSON")
...
def build_chapter_keyboard():
    ...
    for i, chapter in enumerate(get_chapters()):
        buttons.append([
            InlineKeyboardButton(chapter.get("chapter", f"الفصل {i+1}"), callback_data=f"chapter:{i}")
        ])
...
async def send_question(message, context: ContextTypes.DEFAULT_TYPE):
    ...
    await message.reply_text(question_text, reply_markup=InlineKeyboardMarkup(keyboard))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("chapter:"):
        ...
        if not questions:
            await query.edit_message_text("لا توجد أسئلة في هذا الفصل.")
            return
